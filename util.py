...
class GenData(Dataset):
    ...
    def __init__(self, file):
        ...
        # (generate_num, sequence_len)
        self.data = np.genfromtxt(file, dtype='int', delimiter=',')
        N, seq_len = self.data.shape
        zeros = np.zeros(shape=(N, 1), dtype='int')
        # (generate_num, sequence_len+1)
        inputs = np.concatenate((zeros, self.data), axis=1)
        # (generate_num, sequence_len+1)
        targets = np.concatenate((self.data, zeros), axis=1)
