...
class Generator(nn.Module):
    ...
    Embedding -> LSTM -> Linear
      ...
      -LSTM-> (batch_size, seq_len, dim_hidden)
      -Linear-> (batch_size, seq_len, vocab_size)
    """

    def __init__(self, vocab_size, embedding_size, hidden_dim, num_layers):
        super(Generator, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_size)
        self.lstm = nn.LSTM(input_size=embedding_size, hidden_size=hidden_dim, num_layers=num_layers, batch_first=True)
    ...
    def sample(self, batch_size, sequence_len, x=None):
        """
        Given one(or multiple) tokens, generate the next `sequence_len` output tokens

        x: (batch_size, None) some tokens for each sample
        """
